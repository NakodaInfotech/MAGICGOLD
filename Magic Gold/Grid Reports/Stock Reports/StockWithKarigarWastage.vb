...
Public Class StockWithKarigarWastage

    Private Sub cmdexit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdexit.Click
        Me.Close()
    End Sub

    Private Sub StockWithKarigarWastage_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub StockWithKarigarWastage_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            FILLGRID()
            FILLLOSS()
        Catch ex As Exception
    ...
    Sub FILLLOSS()
        Try
            ...
            tempconn.Open()
            tempcmd = New OleDbCommand(" SELECT ROUND(SUM(ACCOUNTMASTER.ACCOUNT_GROSSWT),3) AS LOSSWT FROM ACCOUNTMASTER WHERE (((ACCOUNTMASTER.ACCOUNT_TYPE)='A') AND ((ACCOUNTMASTER.ACCOUNT_BALORJAMAORPAID)='Balance') AND ((ACCOUNTMASTER.ACCOUNT_LEDGERCODE)='LOSS AC')) UNION ALL SELECT ROUND(SUM(ACCOUNTMASTER.ACCOUNT_GROSSWT)*(-1),3) AS LOSSWT FROM ACCOUNTMASTER WHERE (((ACCOUNTMASTER.ACCOUNT_TYPE)='A') AND ((ACCOUNTMASTER.ACCOUNT_BALORJAMAORPAID)='Jama') AND ((ACCOUNTMASTER.ACCOUNT_LEDGERCODE)='LOSS AC')) UNION ALL SELECT ROUND(SUM(mfgWastage.mfg_loss),3) AS LOSS FROM mfgWastage WHERE (((mfgWastage.mfg_loss)<>0))", tempconn)
            ...
            If ClientName = "SHASHAWAT" Or ClientName = "SANGAM" Then
                TXTTOTALGROSS.Text = Format(Val(GGROSSWT.SummaryText) + Val(GWASTAGEGROSS.SummaryText) + Val(GKARIGARGROSS.SummaryText), "0.000")
            Else
                TXTTOTALGROSS.Text = Format(Val(GGROSSWT.SummaryText) + Val(GWASTAGEGROSS.SummaryText) + Val(GKARIGARGROSS.SummaryText) + Val(TXTLOSS.Text.Trim), "0.000")
            End If
    ...
    Sub FILLGRID()
        ...
        If tempconn.State = ConnectionState.Open Then tempconn.Close()
        tempconn.Open()
        If ClientName = "SKYLINE" Or ClientName = "KHUSHALI" Or ClientName = "SHWETA" Or ClientName = "ADITYA" Then
            tempcmd = New OleDbCommand("SELECT STOCKS.ITEM_CODE AS ITEMCODE, '' AS NARRATION, STOCKS.ITEM_PURITY AS PURITY, ROUND(Sum(STOCKS.ITEM_GROSSWT),3) AS GROSSWT, ROUND(Sum(STOCKS.ITEM_NETTWT),3) AS NETTWT, ROUND(Sum(STOCKS.LESSWT),3) AS LESSWT, ROUND(Sum(STOCKS.GROSSLESS),3) AS GROSSLESS FROM Stocks WHERE 1=1 " & WHERECLAUSE & " GROUP BY STOCKS.ITEM_CODE, STOCKS.ITEM_PURITY " & GROUPBYCLAUSE & " HAVING(ROUND(Sum(STOCKS.ITEM_GROSSWT),2) <> 0 Or ROUND(Sum(STOCKS.ITEM_NETTWT),2)) <> 0 ORDER BY STOCKS.ITEM_CODE", tempconn)
        Else
        ...
        If LEDGERDT.Rows.Count > 0 Then
            ...
            If tempconn.State = ConnectionState.Open Then tempconn.Close()
            tempconn.Open()
            tempcmd = New OleDbCommand(" SELECT ROUND(SUM(CDBL(VAL(WASTAGE))) - SUM(CDBL(VAL(JAMAGROSSWT))),3) AS GROSSWT, LEDGERNAME AS WASTAGE FROM WASTAGEREPORT WHERE 1 = 1 " & WHERECLAUSE & " GROUP BY LEDGERNAME " & GROUPBYCLAUSE, tempconn)
            da = New OleDbDataAdapter(tempcmd)
            da.Fill(WASTAGEDT)
        ...
        If tempconn.State = ConnectionState.Open Then tempconn.Close()
        tempconn.Open()
        'Dim tempcmd As New OleDbCommand(" SELECT ROUND(SUM(CDBL(VAL(VACCUMWT))) - SUM(CDBL(VAL(JAMAGROSSWT))),3) AS GROSSWT, 'VACCUM AC' AS NAME FROM VACCUMREPORT ", tempconn)
        ...
        If tempconn.State = ConnectionState.Open Then tempconn.Close()
        tempconn.Open()
        tempcmd = New OleDbCommand(" SELECT ROUND(SUM(ACCOUNTMASTER.ACCOUNT_GROSSWT),3) AS SAMPLEWT, 'SAMPLE AC' AS NAME FROM ACCOUNTMASTER WHERE (((ACCOUNTMASTER.ACCOUNT_TYPE)='A') AND ((ACCOUNTMASTER.ACCOUNT_BALORJAMAORPAID)='Balance') AND ((ACCOUNTMASTER.ACCOUNT_LEDGERCODE)='SAMPLE AC')) UNION ALL SELECT ROUND(SUM(ACCOUNTMASTER.ACCOUNT_GROSSWT)*(-1),3) AS LOSSWT, 'SAMPLE AC' AS NAME FROM ACCOUNTMASTER WHERE (((ACCOUNTMASTER.ACCOUNT_TYPE)='A') AND ((ACCOUNTMASTER.ACCOUNT_BALORJAMAORPAID)='Jama') AND ((ACCOUNTMASTER.ACCOUNT_LEDGERCODE)='SAMPLE AC')) UNION ALL SELECT ROUND(SUM(mfgWastage.SAMPLE),3) AS LOSS, 'SAMPLE AC' AS NAME FROM mfgWastage WHERE (((mfgWastage.SAMPLE)<>0))", tempconn)
        da = New OleDbDataAdapter(tempcmd)
        da.Fill(SAMPLEDT)
        ...
        tempcmd = New OleDbCommand(" SELECT T.NAME, ROUND(SUM(CR) - SUM(DR),3) AS GROSSWT, ROUND(SUM(NETTCR) - SUM(NETTDR),3) AS NETTWT FROM (SELECT ACCOUNT_LEDGERCODE AS NAME, SUM(ACCOUNT_GROSSWT) AS DR, 0 AS CR, SUM(GROSSLESS) AS NETTDR, 0 AS NETTCR FROM ACCOUNTMAST INNER JOIN LEDGERMASTER ON ACCOUNTMAST.ACCOUNT_LEDGERCODE = LEDGERMASTER.LEDGER_CODE WHERE LEDGERMASTER.LEDGER_KARIGAR = TRUE AND ACCOUNT_BALORJAMAORPAID= 'Jama' AND ACCOUNT_DATE <= #" & Format(TILLDATE.Value.Date, "MM/dd/yyyy") & "# GROUP BY ACCOUNT_LEDGERCODE UNION ALL SELECT LEDGER_CODE AS NAME, ROUND(LEDGER_OPBALGROSSWT,3) AS DR, 0 AS CR, 0 AS NETTDR, 0 AS NETTCR FROM LEDGERMASTER WHERE LEDGER_DRCRGROSSWT= 'Cr.' AND LEDGERMASTER.LEDGER_KARIGAR = TRUE UNION ALL SELECT LEDGER_CODE AS NAME, 0 AS DR, ROUND(LEDGER_OPBALGROSSWT,3) AS CR, 0 AS NETTDR, 0 AS NETTCR FROM LEDGERMASTER WHERE LEDGER_DRCRGROSSWT= 'Dr.' AND LEDGERMASTER.LEDGER_KARIGAR = TRUE UNION ALL SELECT ACCOUNT_LEDGERCODE AS NAME, 0 AS DR, SUM(ACCOUNT_GROSSWT) AS CR, 0 AS NETTDR, SUM(GROSSLESS) AS NETTCR FROM ACCOUNTMAST INNER JOIN LEDGERMASTER ON ACCOUNTMAST.ACCOUNT_LEDGERCODE = LEDGERMASTER.LEDGER_CODE WHERE LEDGERMASTER.LEDGER_KARIGAR = TRUE AND ACCOUNT_BALORJAMAORPAID = 'Balance'  AND ACCOUNT_DATE <= #" & Format(TILLDATE.Value.Date, "MM/dd/yyyy") & "# GROUP BY ACCOUNT_LEDGERCODE) AS T  WHERE NAME <> '' GROUP BY T.NAME", tempconn)
        ...
        If ClientName = "SHASHAWAT" Or ClientName = "SANGAM" Or ClientName = "ADITYA" Then
            TXTTOTALGROSS.Text = Format(Val(GGROSSWT.SummaryText) + Val(GWASTAGEGROSS.SummaryText) + Val(GKARIGARGROSS.SummaryText), "0.000")
        Else
            TXTTOTALGROSS.Text = Format(Val(GGROSSWT.SummaryText) + Val(GWASTAGEGROSS.SummaryText) + Val(GKARIGARGROSS.SummaryText) + Val(TXTLOSS.Text.Trim), "0.000")
        End If
...
LINE2:
    ...
    Private Sub StockWithKarigarWastage_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            If ClientName <> "KHUSHALI" And ClientName <> "SHWETA" Then
                GLESSWT.Visible = False
                GGROSSLESS.Visible = False
            End If
        Catch ex As Exception
            Throw ex
        End Try
