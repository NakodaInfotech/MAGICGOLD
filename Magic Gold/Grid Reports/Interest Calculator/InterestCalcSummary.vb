...
Public Class InterestCalcSummary
        ...
        Try
            ...
            If NAMEDT.Rows.Count > 0 Then
                ...
                For Each NAMEROW As DataRow In NAMEDT.Rows
                    ...
                    If tempconn.State = ConnectionState.Open Then tempconn.Close()
                    tempconn.Open()
                    tempcmd = New OleDbCommand("SELECT 1 AS SORTNO, SRNO, TYPE, NAME, DATE, DEBIT, CREDIT, REMARKS, 0 AS [DAYS], 0.0 AS NETTBALANCE, 0.0 AS TOPAY, 0.0 AS TOREC FROM INTERESTVIEW WHERE TYPE <> 'OPENING' " & WHERE & " AND NAME = '" & NAMEROW("NAME") & "' ORDER BY NAME, DATE  ", tempconn)
                    da = New OleDbDataAdapter(tempcmd)
                    da.Fill(dt)
                    ...
                    If tempconn.State = ConnectionState.Open Then tempconn.Close()
                    tempconn.Open()
                    tempcmd = New OleDbCommand(" SELECT IIF((SUM(DEBIT) - SUM(CREDIT)> 0),(SUM(DEBIT) - SUM(CREDIT)),0)AS DEBITBAL, IIF((SUM(CREDIT) - SUM(DEBIT)> 0),(SUM(CREDIT) - SUM(DEBIT)),0) AS CREDITBAL FROM INTERESTVIEW WHERE 1=1 " & OPWHERE & " AND NAME = '" & NAMEROW("NAME") & "'", tempconn)
                    da = New OleDbDataAdapter(tempcmd)
                    da.Fill(DTOPENING)
    ...
    Private Sub InterestCalcSummary_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Sub fillcmb()
        Try
            fillgroup(Me, cmbgroup, cmbgroupid, "")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub

    Private Sub InterestCalcSummary_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            fillcmb()
        Catch ex As Exception
            Throw ex
    ...
    Private Sub GRIDNAMEDETAILS_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            showform()
        Catch ex As Exception
            Throw ex
