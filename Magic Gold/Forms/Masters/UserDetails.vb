...
Public Class UserDetails
    ...
    Sub fillgrid(ByVal name As String)
        Try
            ...
            cmd = New OleDbCommand(" SELECT DISTINCT User_formName as [FormName], User_add as [Add], User_Edit as [Edit], User_View as [View], User_Delete as [Delete] FROM USERMASTER_RIGHTS INNER JOIN USERMASTER ON USERMASTER.USER_ID = USERMASTER_RIGHTS.USER_ID AND USERMASTER.USER_CMPID = USERMASTER_RIGHTS.USER_CMPID WHERE USERMASTER.USER_NAME = '" & name & "' ORDER BY USER_FORMNAME", OGCONN)
            If OGCONN.State = ConnectionState.Open Then OGCONN.Close()
            OGCONN.Open()
            da = New OleDbDataAdapter(cmd)
            da.Fill(dtuser)
