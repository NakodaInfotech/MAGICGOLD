...
Public Class SelectLedger
    ...
    Sub FILLGRID(ByVal WHERECLAUSE As String)
        Try
            ...
            tempcmd = New OleDbCommand("select LEDGER_NAME, LEDGER_CODE   from ledgermaster INNER JOIN GROUPMASTER ON LEDGERMASTER.LEDGER_GROUPID = GROUPMASTER.GROUP_ID WHERE 1 = 1 " & WHERECLAUSE & " ORDER BY LEDGERMASTER.LEDGER_CODE", tempconn)
            ...
            If TXTNAME.Text.Trim <> "" And cmbname.Text.Trim <> "" Then
                If rbstart.Checked = True Then
                    If cmbname.Text = "Name" Then
                        FILLGRID(" AND LEDGER_NAME like '%" & TXTNAME.Text.Trim & "'")
                    ElseIf cmbname.Text = "Code" Then
                        FILLGRID(" AND LEDGER_CODE like '%" & TXTNAME.Text.Trim & "'")
                    End If
